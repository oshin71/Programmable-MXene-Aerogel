...
def generate_random_compositions(materials, n_comps=30, n_sources_per_comp=None,
                                 mass_loading_selections=[2.5, 5, 7.5, 10],
                                 ga_selections=[0, 1],
    ...
    mls = rng.choice(mass_loading_selections, size=(len(comps), 1), replace=True)
    comps = np.hstack([comps, mls])

    if feas_func is not None:
        warnings.filterwarnings("ignore")
        comps = comps[feas_func(comps)]

    ga = rng.choice(ga_selections, size=(len(comps), 1), replace=True)
    comps = np.hstack([comps, ga])
